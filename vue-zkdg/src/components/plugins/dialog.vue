          ...
          <div class="ui-scroller">
            <div @touchstart.stop.prevent="_touchStart" @touchmove.stop.prevent="_touchMove" @touchend.stop.prevent="_touchEnd">
              <dl i-data="0" style="top: 30px" ref="first" class="first">
                <dd pid="0" :i-data="item.region_id" class="" v-for="(item, index) in areaOneList">{{ item.region_name }}</dd>
              </dl>
            </div>
            <div @touchstart.stop.prevent="_touchStart" @touchmove.stop.prevent="_touchMove" @touchend.stop.prevent="_touchEnd">
              <dl i-data="1" style="top: 30px" ref="second" class="second">
                <dd pid="1" :i-data="item.region_id" class="" v-for="(item, index) in areaTwoList">{{ item.region_name }}</dd>
              </dl>
            </div>
            <div @touchstart.stop.prevent="_touchStart" @touchmove.stop.prevent="_touchMove" @touchend.stop.prevent="_touchEnd">
              <dl i-data="2" style="top: 30px" ref="third" class="third">
                <dd pid="2" :i-data="item.region_id" class="" v-for="(item, index) in areaThreeList">{{ item.region_name }}</dd>
              </dl>
            </div>
          </div>
          <!-- 两条遮罩线 -->
