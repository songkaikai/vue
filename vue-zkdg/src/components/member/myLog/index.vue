<template>
  <div class="log-page" v-scroll="_addList">
    <div class="log-wrapper">
      <!-- 日志列表 -->
      <div class="log-inner" v-if="showList">
        <div class="list-item border-bottom-1px" v-for="(item, index) in accountLogList">
          <p><span class="left">{{ item.add_date }}</span><span class="right red">{{ item.in === '0.00' ? '-' + item.out : '+' + item.in }}</span></p>
          <p><span class="left">{{ item.desc }}</span></p>
        </div>
      </div>
      <!--  -->
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        accountLogList: state => state.accountLogList
      })
    },
    created () {
      // 获取window的高度
    ...
    methods: {
      _getList (type) {
        return this.$store.dispatch('accountLogList', {data: this.dataOption, type});
      },
      // 拉倒底部加载
      _addList () {
        if (this.getAgain && this.chooseAgain) {
          ...
          var targetHei = parseInt($('.log-page').height());
