<template>
  <div class="withdrawbank-page" ref="withdraw" v-scroll="_addList">
    ...
    <div class="withdraw-wrapper wrapper-absolute" v-if="chooseType === 0">
      ...
      <p class="all-money">当前账号可提现收益：{{ parseInt(memberInfo.integral_balance).toFixed(2) }}</p>
      <p class="all-money" style="color:red">*提现金额500起提，100的倍数</p>
    </div>
    <!-- 提现记录 -->
    <div class="withdraw-wrapper" v-if="chooseType === 1">
      <div class="inner-wrapper" v-if="showLists">
        ...
        <div class="list-wrapper" style="padding: 0 0.8rem">
          <div class="list-item" :class="[bankwraList.length - 1 > index ? 'border-bottom-1px' : '']" v-for="(item, index) in bankwraList">
            <p class="title">
            ...
            <p class="title" style="margin-bottom:0;">
              ...
              <span class="money">{{ item.statusName }}</span>
            </p>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: mapState({
      ...
      // 提现日志接口
      bankwraList: state => state.bankwraList
    }),
    methods: {
      _chooseType (type) {
      ...
      _subList () {
        ...
        return this.$store.dispatch('bankwraList', this.listOption);
      },
       // 拉倒底部加载
      _addList () {
