<template>
  <div class="teamlist-page" ref="tjlist" v-scroll="_addList">
    <div v-if="showLists">
     ...
     <div class="list-item" :class="[teamJtCountList.length - 1 > index ? 'border-bottom-1px' : '' ]" v-for="(item, index) in teamJtCountList">
        <p class="desc">
          <span class="phone">{{ item.mobile }}</span>
          <span class="count" style="color: #282828">{{ item.nick_name }}</span>
        </p>
        <p class="desc" style="margin-bottom:0">
          <span class="phone">{{ item.uname }}</span>
          <span class="count">{{ item.jobs | formatDate }}</span>
        </p>
...
<script>
  ...
  export default {
    ...
    computed: mapState({
      ...
      // 岗位津贴列表日志
      teamJtCountList: state => state.teamJtCountList
    }),
    created () {
      // 获取window的高度
    ...
    methods: {
      ...
      _subList () {
        ...
        return this.$store.dispatch('teamJtCountList', this.listOption);
      },
       // 拉倒底部加载
      _addList () {
        if (this.getAgain && this.chooseAgain) {
          ...
          var targetHei = parseInt($('.teamlist-page').height());
