<template>
  <div class="xf-list-page" ref="tjlist" v-scroll="_addList">
    <div v-if="showLists">
     ...
     <div class="list-item border-bottom-1px" v-for="(item, index) in xfCountList">
        <p class="desc">
          <span class="phone">{{ item.add_time }}</span>
          <span class="count">{{ item.order_money }}</span>
        </p>
        <p class="name" style="text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;margin-bottom:0.2rem">{{ item.order_id }}</p>
        <p class="name" style="text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;">{{ item.status_name }}</p>
      </div>
    </div>
    <!-- 没有数据时显示 -->
...
<script>
  ...
  export default {
    ...
    computed: mapState({
      ...
      // 岗位津贴列表日志
      xfCountList: state => state.xfCountList
    }),
    created () {
      // 获取window的高度
    ...
    methods: {
      ...
      _subList () {
        ...
        return this.$store.dispatch('xfCountList', this.listOption);
      },
       // 拉倒底部加载
      _addList () {
        if (this.getAgain && this.chooseAgain) {
          ...
          var targetHei = parseInt($('.xf-list-page').height());
