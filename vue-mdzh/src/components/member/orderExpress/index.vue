<template>
  <div class="express-page">
      <div class="expree-header" v-if="showExpress">
        <p class="name">快递公司：<span>{{ expressInfo.ShipperName }}</span></p>
        <p class="order">快递单号：<span>{{ expressInfo.LogisticCode }}</span></p>
        <!-- <p class="status">当前状态：<span>{{ expressInfo.ShipperName }}</span></p> -->
      </div>
...
<script>
  ...
  export default {
    ...
    created () {
       this.dataOption.expressNo = this.$route.query.expressNo;
       this.dataOption.express = this.$route.query.express;
      this.$nextTick(() => {
        this._getMsg();
      });
    },
