    ...
    <div class="address-wrapper" v-if="addressList[0]">
      <div class="list-item border-bottom-1px" v-for="(item, index) in addressList" >
        <div class="inner-msg">
            <div class="one-line">
              <i class="icon-one"></i>
              <p class="address">{{ item.area_name | formatDate }} {{ item.address }}</p>
            </div>
            <div class="two-line">
  ...
  export default {
    ...
    methods: {
      ...
      submitMsg () {
        // 弹框确定回调按钮
        this.$refs.loading.showLoad();
        this.$store.dispatch('delAddress', {token: this.memberLogin.token, addressId: this.delId}).then(res => {
            this.$refs.loading.hideLoad();
            if (res.code === 200) {
              this.getList();
      ...
      chooseDefault (id) {
        this.$refs.loading.showLoad();
        this.$store.dispatch('chooseDefault', {token: this.memberLogin.token, addressId: id}).then(res => {
            this.$refs.loading.hideLoad();
            if (res.code === 200) {
              this.getList();
              this._showTip('修改成功');
...
<style>
  .address-page{background: #fff;}
  .address-page .add-btn{position: fixed;bottom: 0;left:0;width:100%;background: #000;text-align: center;padding: 0.5rem 0;z-index: 111;}
  ...
  .address-page .add-btn span{display: inline-block;vertical-align: middle;font-size: 0.9rem;color:#cbb693;margin-left: 0.3rem;}
  .address-page .address-wrapper{padding-bottom: 2.6rem;color:#555555;}
  ...
  .address-page .address-wrapper .inner-msg .address{font-size: 0.9rem;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;margin-top: 0.2rem;}
  ...
  .address-page .address-wrapper .edit-all .left-choose i{vertical-align: middle;display: inline-block;width: 1.5rem;height:1.5rem;background: url(./address_no.png) no-repeat;background-size: 100% 100%;}
  .address-page .address-wrapper .edit-all .left-choose i.index{background: url(./address_default.png) no-repeat;background-size: 100% 100%;}
